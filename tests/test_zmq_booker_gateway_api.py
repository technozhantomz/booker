...
import zmq
from aiozmq import ZmqStream, create_zmq_stream

from booker.dto import DataTransferClass, Amount
from booker.rpc.api import APIStream, api_method, api_client, api_server
from booker.rpc.zmq_jsonrpc_api import (
    ZMQJSONRPCAPIsClient,
    ZMQJSONRPCAPIsServer
)
...
async def test_new_order() -> None:
    ...
    booker_server = MockBookerGatewayOrderAPIServer(booker=booker)

    apis_server.api_register(booker_server)

    address = list(server_stream.transport.bindings())[0]

    async def eth_booker_client_zmq_stream_constructor(
    ) -> ZmqStream:
        zmq_stream = await create_zmq_stream(
            zmq.REQ,
            connect=address
        )
    ...
    eth_booker_apis_client = ZMQJSONRPCAPIsClient(
        stream_constructor=eth_booker_client_zmq_stream_constructor
    )
    ...
    async def eth_gateway_client_zmq_stream_constructor(
    ) -> ZmqStream:
        zmq_stream = await create_zmq_stream(
            zmq.REQ,
            connect=address
    ...
    eth_apis_client = ZMQJSONRPCAPIsClient(
        stream_constructor=eth_gateway_client_zmq_stream_constructor
    )
    ...
    async def bts_booker_client_zmq_stream_constructor(
    ) -> ZmqStream:
        zmq_stream = await create_zmq_stream(
            zmq.REQ,
            connect=address
    ...
    bts_booker_apis_client = ZMQJSONRPCAPIsClient(
        stream_constructor=bts_booker_client_zmq_stream_constructor
    )
    ...
    async def bts_gateway_client_zmq_stream_constructor(
    ) -> ZmqStream:
        zmq_stream = await create_zmq_stream(
            zmq.REQ,
            connect=address
    ...
    bts_apis_client = ZMQJSONRPCAPIsClient(
        stream_constructor=bts_gateway_client_zmq_stream_constructor
    )
    ...
    with pytest.raises(StopAsyncIteration):
        await bts_validate_address.asend(None)

    eth_get_deposit_address = eth_gateway_client.get_deposit_address(
        GetDepositAddress(out_tx_to=new_deposit_in_order.out_tx_to)
    )
    eth_deposit_address = await eth_get_deposit_address.asend(None)

    assert eth_deposit_address.tx_to == new_deposit_in_order.in_tx_to

    with pytest.raises(StopAsyncIteration):
        await eth_get_deposit_address.asend(None)

    eth_new_in_order = eth_gateway_client.new_in_order(new_deposit_in_order)

    assert await eth_new_in_order.asend(None) == None

    with pytest.raises(StopAsyncIteration):
        await eth_new_in_order.asend(None)

    bts_new_out_order = bts_gateway_client.new_out_order(
        new_deposit_out_order
    )

    assert await bts_new_out_order.asend(None) == None

    with pytest.raises(StopAsyncIteration):
        await bts_new_out_order.asend(None)

    bts_get_deposit_address = bts_gateway_client.get_deposit_address(
        GetDepositAddress(out_tx_to=new_withdrawal_in_order.out_tx_to)
    )
    bts_deposit_address = await bts_get_deposit_address.asend(None)

    assert bts_deposit_address.tx_to == new_withdrawal_in_order.in_tx_to

    with pytest.raises(StopAsyncIteration):
        await bts_get_deposit_address.asend(None)

    bts_new_in_order = bts_gateway_client.new_in_order(
        new_withdrawal_in_order
    )

    assert await bts_new_in_order.asend(None) == None

    with pytest.raises(StopAsyncIteration):
        await bts_new_in_order.asend(None)

    eth_new_out_order = eth_gateway_client.new_out_order(
        new_withdrawal_out_order
    )

    assert await eth_new_out_order.asend(None) == None

    with pytest.raises(StopAsyncIteration):
        await eth_new_out_order.asend(None)

    await asyncio.gather(
        eth_server_process_orders_task,
        bts_server_process_orders_task
    )

...
from finteh_proto.dto import OrderDTO, DepositAddressDTO, ValidateAddressDTO, TransactionDTO
...
class BookerSideClient(BaseClient):
    ...
    def __init__(self, gateway_name="AbstractGateway", gateway_side="Not spec", ctx=None, host="0.0.0.0", port=8080):
    ...
    async def init_new_tx_request(self, order: OrderDTO) -> TransactionDTO:
        ...
        call_result = await self.call('init_new_tx', id=str(uuid4()), params=order.to_dump())
    ...
    async def get_deposit_address_request(self, deposit_address: DepositAddressDTO):
        ...
        call_result = await self.call('get_deposit_address', id=str(uuid4()), params=deposit_address.to_dump())
        resp_data = json.loads(call_result[1]["params"])
        return DepositAddressDTO(**resp_data)

    @BaseClient.safe_call_execute
    async def validate_address_request(self, user_address: ValidateAddressDTO):
        call_result = await self.call('validate_address', id=str(uuid4()), params=user_address.to_dump())
        resp_data = json.loads(call_result[1]["params"])

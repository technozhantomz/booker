...
class BookerServer(BaseServer):
    ...
    async def create_order(self, request):
        ...
        out_tx = TransactionDTO(amount=in_tx.amount,
                                tx_id="MOCK_OUT_HASH",
                                coin=in_tx.coin,
        ...
        order = OrderDTO(order_id=str(uuid4()), in_tx=in_tx, out_tx=out_tx, order_type=order_type)
        ...
        if self.ctx:
            async with self.ctx.db_engine.acquire() as conn:
                ...
                order_model = Order(id=order.order_id,
    ...
    async def update_tx(self, request):
        ...
        msg = JsonRpcMsg(type=JsonRpcMsgTyp.RESPONSE,
                         data={"params": updated_order.to_dump()})

...
from booker.booker_gateway_api import OrderType, TxStatus, TxError
...
class Tx(Base):
    ...
    amount = sa.Column(sa.Numeric(78, 36))
    created_at = sa.Column(
        sa.DateTime,
        nullable=False,
        server_default=sa.func.current_timestamp()
    )
    status = sa.Column(sa.Enum(TxStatus), nullable=False, server_default='WAIT')
    error = sa.Column(
        sa.Enum(TxError),
        nullable=False,

...
async def new_in_order(context: AppContext, args: NewInOrder) -> None:
    ...
    query = sa.update(Order).where(where).values(
        party=GatewayParty.IN_CREATED
    ).returning(Order.id)
...
async def new_out_order(context: AppContext, args: NewOutOrder) -> None:
    ...
    query = sa.update(Order).where(where).values(
        party=GatewayParty.OUT_CREATED
    ).returning(Order.id)
...
async def new_in_order_request(
    ...
    out_tx_id = uuid4()
    out_tx_query = sa.insert(Tx).values(
        id=out_tx_id,
...
async def new_in_tx_order(context: AppContext, args: NewInTxOrder) -> None:
    ...
    update_in_tx_where = (
        ...
        ((OutTx.c.to_address == None) |
         (OutTx.c.to_address == args.memo_to)) &
        (OutTx.c.error == TxError.NO_ERROR) &
        (OutTx.c.confirmations == 0) &
        (OutTx.c.max_confirmations == 0)
    )
    update_in_tx_query = sa.update(Tx).values(
        tx_id=args.tx_hash,
    ...
    ).where(update_in_tx_where).returning(Order.id)
    update_out_tx_where = (Order.id == args.order_id) & (Order.out_tx == Tx.id)
    update_out_tx_query = sa.update(Tx).values(to_address=args.memo_to).where(
        update_out_tx_where
    )
...
async def update_in_tx_order(context: AppContext, args: UpdateTxOrder) -> None:
    ...
    query = sa.update(Tx).where(where).values(
        error=args.tx_error,
        confirmations=args.tx_confirmations,
        max_confirmations=args.tx_max_confirmations
    ).returning(Order.id)
